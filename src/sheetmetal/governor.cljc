(ns sheetmetal.governor
  "SheetMetalWorkerGovernor — the independent safety/scope layer gating
  every workshop scheduling/logistics proposal an advisor may make for
  a sheet-metal fabrication crew. The governor never dispatches
  hardware itself, never performs metalworking on the shop floor, and
  never finalizes a metal-cutting/forming-execution decision (e.g.
  deciding to proceed with a specific cutting or press-brake forming
  operation) or overrides a shop safety officer's judgment — those are
  ...
  performs sheet-metal work itself). Modeled on
  cloud-itonami-isco-7111's housebuilder.governor (and closely on
  ...
  HARD invariants (:hard? true, ALWAYS :hold, never overridable):
    ...
    3. no-actuation           — proposal :effect must be :propose (the
                                ...
                                never performs metalworking itself; it
    ...
    5. scope-excluded action  — any proposal to directly finalize a
                                metal-cutting/forming-execution
                                decision (e.g. deciding to proceed with
                                a specific cutting or press-brake
                                forming operation), or to override a
                                ...
                                metal-cutting operation\" / \"authorize
                                the press brake operation\" /
                                \"override the shop safety officer's
                                judgment\", never as bare nouns like
                                \"cutting\", \"forming\" or \"safety\",
                                ...
                                cutting task …\" or \"…routed for shop
  ...
  regardless of confidence):
    6. :op :flag-safety-concern (a cut-hazard / machine-press-hazard /
...
(def ^:private scope-excluded-ops
  #{:finalize-cutting-decision :authorize-forming-operation
    :proceed-with-metal-cutting-work :override-safety-officer-judgment
    :override-shop-safety-officer-judgment})

;; Full finalization/execution ACTION phrases only — never bare nouns
;; ("cutting", "forming", "press brake", "safety", "shop safety
...
(def ^:private scope-excluded-phrases
  ["proceed with the metal-cutting operation" "proceed with the metal-cutting work"
   "proceed with the forming operation" "proceed with the press brake operation"
   "finalize the cutting decision" "finalize the forming decision"
   "authorize the forming operation" "authorize the press brake operation"
  ...
  (let [{:keys [op rationale]} proposal]
    (cond-> []
      ...
      (conj {:rule :scope-excluded-action
             :detail "金属切断/成形工程の実行判断の確定・shop safety officer の判断の上書きは、この actor の権限外 — 常に永続ブロック"}))))

(defn check
  "Assess a proposal against `request`/`context`/`proposal` and a
  `store` implementing `sheetmetal.store/Store`. Pure — never
  mutates the store, never dispatches a shop-floor operation."
