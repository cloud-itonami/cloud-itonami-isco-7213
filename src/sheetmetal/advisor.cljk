(ns sheetmetal.advisor
  "Sheet Metal Worker Advisor — proposing a workshop scheduling/logistics
  coordination operation (log a work record, schedule a crew
  operation, flag a safety concern, coordinate a sheet-metal-materials
  supply order) from a crew roster, workshop registration and
  safety-reporting policy. Swappable mock/llm; the advisor ONLY
  proposes — `sheetmetal.governor` independently gates every
  ...
  metal-cutting/forming-execution decision (e.g. a specific cutting or
  press-brake forming operation) or to override a shop safety
  officer's judgment — those stay permanently out of this actor's
  scope. Modeled on cloud-itonami-isco-7111's advisor (and closely on
...
(defn- rationale-for [op worker-id workshop-id hazard-type]
  (case op
    ...
    (str "scheduled crew operation for cutting task at workshop " workshop-id)
...
(def ^:private system-prompt
  "You are a sheet-metal workshop scheduling/logistics coordination
   ...
   propose to directly finalize a metal-cutting/forming-execution
   ...
   metalworking or authorizes cutting/forming operations itself.
